"""
Download Costa Rica National Parks data from Wikipedia
This script scrapes Wikipedia for Costa Rica National Parks information
"""
...
def get_costa_rica_parks_data():
    """
    Scrape Costa Rica National Parks from Wikipedia
    Returns a list of park dictionaries
    """
    parks = []
    
    # Wikipedia page for Costa Rica National Parks
    url = "https://en.wikipedia.org/wiki/List_of_national_parks_of_Costa_Rica"
    
    try:
        print("Fetching Costa Rica National Parks data from Wikipedia...")
            ...
            for row in rows[1:]:
                ...
                if len(cells) >= 2:
                    try:
                        ...
                        if not name or name.lower() in ['name', 'park name', 'province', 'area', '']:
                            continue
                        
                        # Extract province/region (usually second column)
                        ...
                        if name and name.lower() not in ['name', 'park name']:
                            park = {
                                ...
                                'Description': f"Costa Rica National Park in {province}. Area: {area}" if area else f"Costa Rica National Park in {province}",
                                'URL': '',
                                'Country': 'Costa Rica'
                            }
                            parks.append(park)
                            print(f"  Found: {name} ({province})")
                            
                    except Exception as e:
                        print(f"  Error processing row: {e}")
                        continue
        
        print(f"\nTotal Costa Rica National Parks found: {len(parks)}")
        return parks
...
def save_to_csv(parks, filename='Costa_Rica_National_Parks.csv'):
...
if __name__ == '__main__':
    print("=" * 60)
    print("Costa Rica National Parks Data Downloader")
    print("=" * 60)
    
    parks = get_costa_rica_parks_data()
    ...
    if parks:
        ...
        csv_path = os.path.join(data_dir, 'Costa_Rica_National_Parks.csv')
        ...
        public_csv_path = os.path.join(public_data_dir, 'Costa_Rica_National_Parks.csv')
